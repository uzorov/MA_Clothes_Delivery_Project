...
auth_url = "http://158.160.14.173:8000/auth/login"
...
def set_discount_to_order(code: str, order_id: UUID, request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="promocode",
                             path=f"/promocode/discount?code={code}&id={order_id}&clear=false", user_info=current_user,
                             request=request)
...
def post_item_to_cart(
    ...
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="item",
                             path=f"/item/add_to_cart?item_id={item_id}&size={str(size.value)}&count={count}",
                             user_info=current_user, request=request)
...
def get_user_payments(request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="payment", path=f"/payments/get-user-payments", user_info=current_user,
                             request=request)
...
def process_payment(id: UUID, request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="payment", path=f"/payments/{id}/process", user_info=current_user,
                             request=request)
...
def choose_pickup(id: UUID, request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="delivery", path=f"/delivery/{id}/choose_pickup", user_info=current_user,
                             request=request)
...
def choose_delivery(id: UUID, request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="delivery", path=f"/delivery/{id}/choose_delivery", user_info=current_user,
                             request=request)
...
def begin_printing(id: UUID, request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="printing", path=f"/printing/{id}/begin", user_info=current_user,
                             request=request)
...
def finish_printing(id: UUID, request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="printing", path=f"/printing/{id}/finish", user_info=current_user,
                             request=request)
...
def cancel_printing(id: UUID, request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="printing", path=f"/printing/{id}/cancel", user_info=current_user,
                             request=request)
...
def activate_delivery(id: UUID, request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="delivery", path=f"/delivery/{id}/activate", user_info=current_user,
                             request=request)
...
def finish_delivery(id: UUID, request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="delivery", path=f"/delivery/{id}/finish", user_info=current_user,
                             request=request)
...
def cancel_delivery(id: UUID, request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="delivery", path=f"/delivery/{id}/cancel", user_info=current_user,
                             request=request)
...
def create_item(name: str,
    ...
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="item", path=f"/item/?name={name}&price={price}&design={design}",
                             user_info=current_user, request=request)
...
def delete_item(id: UUID, request: Request, current_user: dict = Depends(get_user_role)):
    if current_user['id'] == '':
        ...
        return proxy_request(service_name="item", path=f"/item/delete_item/{id}", user_info=current_user,
                             request=request)
